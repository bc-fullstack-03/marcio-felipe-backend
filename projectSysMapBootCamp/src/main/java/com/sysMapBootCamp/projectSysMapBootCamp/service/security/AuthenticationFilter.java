...
public class AuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (request.getServletPath().contains("/api/v1/authentication/login")
                || request.getServletPath().contains("/api/v1/user/createUser")
                || request.getServletPath().contains("api/v1/post/getAllPost")
        ) {
        ...
        try {
            isValidToken = _jwtService.isValid(token.substring(7),userId);
